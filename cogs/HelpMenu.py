...
class HelpMenu(commands.Cog, name='Help'):
    ...
    async def help(self, ctx):
        if ctx.invoked_subcommand is None:
            ...
            embed.add_field(
                ...
                value='Управление сервером',
                inline=False
            )
            embed.add_field(
                name='**Music**',
                # value='The music player',
                value='Музыкальный плеер',
                inline=False
            )
            embed.add_field(
                name='**Other**',
                # value='The music player',
                value='Другие функции',
                inline=False
            )
    ...
    async def help_playlist(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description='**Список функций плейлиста**\n\nИспользуйте `.help playlist [option]` для дополнительной информации',
            timestamp=ctx.message.created_at
        )
        extras = guildcol.find_one({'guild_id': ctx.guild.id})['prefixes']
        embed.set_footer(
        ...
        embed.add_field(
            ...
            value='Description: Показывает список доступных плейлистов на сервере\nUsage: `.playlist`\nAliases: `.pl, .plist`'
        )
    ...
    @help.command(
        name='Other',
        aliases=['other', ],
        description='Показать список других команд',
    )
    async def Other(self, ctx):
        cog = self.client.get_cog('Other')
        commands = cog.get_commands()
        embed = discord.Embed(
            color=discord.Color.orange(),
            description='**List of other commands**\n\nUse `.help [command name]` for more information',
            timestamp=ctx.message.created_at
        )
        embed.set_author(
            name=cog.qualified_name
